...
平台调整后：
- 平台VIP1范围：85% ~ 90%
- 商家A自动调整为：90%（超出范围，自动修正为最大值）

平台调整后：
- 平台VIP1范围：93% ~ 98%
- 商家A自动调整为：93%（低于范围，自动修正为最小值）
```

**调整逻辑：**
```typescript
if (merchantDiscount < newMin) {
  merchantDiscount = newMin  // 低于最小值，调整为最小值
}
if (merchantDiscount > newMax) {
  merchantDiscount = newMax  // 高于最大值，调整为最大值
}
```
...
| 会员折扣范围 | `merchantDiscountMin` ~ `merchantDiscountMax` | 80% ~ 95% | 平台设定（可配置） |
